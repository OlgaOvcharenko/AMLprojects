...
class RNN(nn.Module):
    ...
    def forward(self, x):
        ...
        hidden_h = torch.zeros(self.n_layers, batch_size, self.hidden_size, dtype=torch.float32)
        hidden_c = torch.zeros(self.n_layers, batch_size, self.hidden_size, dtype=torch.float32)

        out_all, (hidden, hidden_c) = self.rnn(c3_t, (hidden_h, hidden_c))

        attention = torch.zeros(batch_size, self.attention_size)
...
def train_rnn(X_train, y_train, n_epochs: int = 20,
              n_hidden: int = 128, lr: float = 0.01,
              batch_size=56, print_iter=2, n_layers=1, output_size=4):
    ...
    X_train, y_train, data_loader = get_all_data(X_train, y_train, batch_size)
    rnn.train(True)
    for epoch in range(n_epochs):
        # X_train_size = X_train.size()
        # permutation = torch.randperm(X_train_size[0])

        current_batch = 0
        for iteration, (batch_x, batch_y) in enumerate(data_loader):
            # indices = permutation[current_batch: current_batch + batch_size]
            # batch_x = X_train[indices]
            # batch_y = y_train[indices]

            current_batch += batch_size
            batch_y = torch.flatten(batch_y).long()
            optimizer.zero_grad()
            output = rnn(batch_x)
