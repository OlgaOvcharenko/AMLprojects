...
def augment(X, Y, id, original_size, seed):
    ...
    gaussian = np.random.normal(mean, sigma, (X.shape[0], X.shape[1])) * random.randint(1, 100)
...
class MyDataset(Dataset):
    def __init__(self, data, augment=True):
        ...
        for video in data:
            for i in range(len(video['frames'])):
                self.data.append(
                    (video['augmented_frames'][i], video['augmented_box'][i]))
    ...
    def __len__(self):
        return len(self.data) * (6 if self.augment else 1)
...
def combine_loss(output, Y):
    ...
    loss += nn.functional.binary_cross_entropy(out_sigmoid, Y)
    loss += dice_loss(Y, out_sigmoid)
    loss += 1 + dice_coef_loss_sq(Y, out_sigmoid)
...
def train(prof_train, val_train, check_point=None,
          learning_rate=0.00001, batch_size=20, epochs=100, print_iteration=40):
    network = UNet(encoder_args=(1, 64, 128, 256),
                   decoder_args=(256, 128, 64))

    model_save_folder = "./Trained_boxes/signal_unet/"
    if not os.path.exists(model_save_folder):
        os.makedirs(model_save_folder)

    if check_point:
        network.load_state_dict(torch.load('Trained_boxes/signal_unet/{}.pth'.format(check_point)))
        start = int(check_point.split("ep-")[1].split(".pth")[0]) + 1
    else:
        start = 0
    ...
    loader = DataLoader(MyDataset(prof_train, augment=True), batch_size=batch_size, num_workers=10, shuffle=True)
    ...
    for epoch in range(start, epochs):
        ...
        for it, (X, Y) in enumerate(loader):
            ...
            if it % print_iteration == 0:
                ...
                print('{:3d} / {:3d} Train loss: {:15.8f}, train accuracy: {:15.4f}, '
                      'validation loss: {:15.8f}, validation accuracy {:1.4f}'
